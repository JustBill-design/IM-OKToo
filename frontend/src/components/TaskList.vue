...
async function fetchTasks() {
  ...
  try {
    const res = await fetch(`https://api-gateway-366297756669.us-central1.run.app/api/tasks?username=${currentUser}`)
...
async function addTask(descriptionFromParent?: string) {
  ...
  try {
    const res = await fetch('https://api-gateway-366297756669.us-central1.run.app/api/tasks', {
      method: 'POST',
...
async function removeTask(idx: number) {
  ...
  try {
    const res = await fetch(`https://api-gateway-366297756669.us-central1.run.app/api/tasks/${task.id}`, { method: 'DELETE' })
